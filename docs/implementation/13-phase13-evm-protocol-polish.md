...
| `evm_stablecoin_decimals(symbol)` 심볼 기반 정적 매핑 | USDC/USDT/DAI 코드 등록 | 신규 토큰 추가 시 코드 변경, 컨트랙트가 다른 decimals 사용하면 침묵 오류 |
...
## 13-3. Stablecoin decimals — 심볼 매핑에서 on-chain 조회로

### Phase 12 상태

Phase 12-5 가 `evm_stablecoin_decimals(symbol: &str) -> Option<u8>` 헬퍼를 추가
했다 (`USDC|USDT → 6`, `DAI → 18`). 심볼 기반 정적 매핑이므로 새 토큰을 등록
하려면 코드 수정이 필요하고, **컨트랙트가 등록값과 다른 decimals 를 쓰면
침묵 오류**가 발생한다 (예: 향후 6→8 마이그레이션, 새 L2 의 변종 토큰).

### 변경 설계 — `fetch_token_meta` 와 같이 통합

13-1 의 `fetch_token_domain` 캐시와 같은 인프라를 재사용해 한 번의 RPC 흐름
에서 도메인 + decimals 를 동시에 가져온다:

```rust
// crates/core/src/client/evm_token_meta.rs
#[derive(Clone, Debug)]
pub struct EvmTokenMeta {
    pub decimals: u8,
    pub eip712_domain: Eip712TokenDomain,  // 13-1 참조
}

static CACHE: OnceLock<RwLock<HashMap<(u64, Address), EvmTokenMeta>>> = OnceLock::new();
...
pub async fn fetch_token_meta(
    ...
    let cache = CACHE.get_or_init(|| RwLock::new(HashMap::new()));
    if let Some(m) = cache.read().await.get(&(chain_id, token)) {
        return Ok(m.clone());
    }
    let provider = ProviderBuilder::new().connect_http(rpc_url.parse()?);
    let token_iface = IErc20Meta::new(token, &provider);
    let domain_iface = IEip712Domain::new(token, &provider);

    // 2 view calls + 1 EIP-5267 → fail-soft per call so missing pieces fall
    // back to the symbol-based defaults.
    let decimals = token_iface.decimals().call().await
        .map_err(|e| format!("decimals() failed: {e}"))?;
    let domain = domain_iface.eip712Domain().call().await
        .map(|d| Eip712TokenDomain { name: d.name, version: d.version })
        .unwrap_or_else(|_| static_fallback_domain(chain_id));

    let meta = EvmTokenMeta { decimals, eip712_domain: domain };
...
| 파일 | 변경 |
|------|------|
| `evm_x402_payment.rs::build_evm_requirements` | `fetch_token_meta(...).decimals` 사용 (현 `evm_stablecoin_decimals` 호출 대체) |
| `client/balance.rs::get_evm_balances` | symbol 기반 헬퍼 대신 token 컨트랙트 별로 캐시 사용 |
| `client/send_evm.rs::send_erc20_async` | 동일 |
| `client/evm.rs::build_evm_payment` | 동일 |

### `evm_stablecoin_decimals` 의 운명

- 심볼 → decimals fallback 으로 유지하되 `Option<u8>` 시그니처는 그대로.
  RPC 가 응답하지 않는 환경(테스트 모킹, 캐시 cold start 시 실패)을 위한
  안전망. 단 정상 경로에서는 항상 on-chain 결과를 우선.
- 새 토큰 등록 시 hardcoded 값은 추가하지 않아도 동작. 다만 fallback 이
  명시되어 있으면 RPC 실패 시 더 친절한 에러.
...
| 파일 | 유형 | 변경 |
|------|------|------|
| `rust/crates/core/src/client/evm_token_meta.rs` | **신규** | `fetch_token_meta` (OnceLock 캐시 — decimals + EIP-712 domain) |
